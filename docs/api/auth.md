...
Refresh Token은 Redis에 저장하며 key는 `refresh:{userId}` 형식을 사용한다. email, loginId, nickname처럼 변경 가능한 사용자 식별자는 Redis key에 사용하지 않는다. TTL은 Refresh Token 만료 시간 정책을 따른다. 다중 기기 로그인 구조는 Phase 9-8 범위에서 다루지 않는다.
